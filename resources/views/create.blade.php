    ...
    <main class="sm:container">
        <div class="w-full">
            <section class="flex flex-col break-words bg-white sm:border-1 sm:rounded-r-lg sm:shadow-sm sm:shadow-lg">
                ...
                <form class="w-full my-6 px-6 space-y-6 sm:my-8 sm:px-8 sm:space-y-8" action="/store" method="POST">
                    @csrf
                    <div class="flex flex-wrap">
                        <label for="note" class="text-gray-700 w-full">
                            <textarea id="note" name="content" class="form-input w-full px-4 py-2 text-2xl text-gray-700 placeholder-gray-400 rounded-lg border-2 border-gray-300 focus:outline-none focus:ring-4 focus:ring-purple-600 focus:border-transparent" rows="10" placeholder="Enter your content here..."></textarea>
                        </label>
                    </div>
